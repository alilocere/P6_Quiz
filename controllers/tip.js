...
exports.create = (req, res, next) => {
    ...
    /*if(authorId===undefined){
        res.redirect("https://localhost:3000/session")
    }*/
...
exports.edit = (req, res, next, tipId) => {

    const{tip, quiz} = req;
    res.render('tips/edit', {tip, quiz});
};

exports.update = (req, res, next, tipId) => {

    const{tip, quiz, body} = req;
    tip.text=body.text;
    tip.accepted=false;

    tip.save({fields: ["texto", "accepted"]})
    .then(tip => {
        req.flash('success', 'Tip accepted successfully.');
        res.redirect('/goback');
    })
    ...
    .catch(error => {
        req.flash('errror', 'Error editing the tip' + error.message);
        next(error);
    });
